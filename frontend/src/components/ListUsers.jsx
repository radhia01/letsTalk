...
const Listusers = () => {
  ...
  const { usermessages, users,token } = useSelector(
    (state) => state.user
  );
  ...
  const addmessagetoGroup = async (e, idgroup) => {
    e.preventDefault();
    const data = { idgroup, iduse, message,token };
  ...
  const sendmessage = async (e, id_receiver) => {
    e.preventDefault();
    const data = { id_receiver, iduse, message ,token};
  ...
  const showUser = async (idus) => {
    ...
    dispatch(setMessageToViewed(iduser1, iduser2,token));
    setuser(userData);
    setshow(true);
    setnewmessage(false);
  };
  ...
  const sendinvitation = async (idfriend) => {
    dispatch(sendInvitation(iduse, idfriend,token));
  ...
  // add new group
  const addgrp = async () => {
    dispatch(addGroup(groupname,token));
  };
  const hover = (id) => {
  ...
  const joingroup = async (idgroup) => {
    dispatch(joinGroup(iduse, idgroup,token));
  };
  // leave group
  const Leavegrp = async (idgroup) => {
    dispatch(leaveGroup(iduse, idgroup,token));
  };

  useEffect(() => {
    dispatch(getUserMessage(token));
    dispatch(getGroups(token));
    dispatch(getGroupsMessages(token));
    dispatch(getInvitations(token));
  ...
  return (
    <div className="homepage">
      ...
      <div className="d-flex col-md-12 ">
        ...
        {/* ShowUser part 2  */}
        <div className="col-md-7 m-4 part2  " style={{maxHeight:700,overflowY:"auto"}}>
          {show === true ? (
