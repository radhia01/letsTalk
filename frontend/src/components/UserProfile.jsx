...
const Userprofile = () => {
  ...
  const iduser =useSelector(state=>state.user.user)._id
  const { users, error, removeAccount ,token,user} = useSelector((state) => state.user)
  ...
  const deleteUser = () => {
    dispatch(deletefriend(idfriend, user._id,token));
  ...
  const deleteaccount = async (e) => {
    e.preventDefault();
    const id = user._id;
    dispatch(deleteAccount(password, id,token));
  };
  const deleteacc = () => {
    setdeleteacc(true);
    seteditpassword(false);
  ...
  return (
    <div className="d-flex  profile  bg-white">
      ...
      <div className="col-md-4   " style={{ marginTop: "50px" }}>
        {editpic && <EditPicture id={iduser} seteditpic={seteditpic} />}
      </div>
